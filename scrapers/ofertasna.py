...
CATEGORIES = [
    ("https://www.ofertasna.com.br/decant/", "decant"),
    ("https://www.ofertasna.com.br/decant-no-frasco/", "decant"),
...
class OfertasNaScraper(BaseScraper):
    store_name = "OfertasNa"
    store_slug = "ofertasna"
    base_url = "https://www.ofertasna.com.br"

    def _parse_tn_price(self, price_str):
        """Handle Tienda Nube prices like 'R$0,00R$19,00' (original+sale concatenated)."""
        matches = re.findall(r"R\$\s*([\d.]+,\d{2})", price_str)
        for m in reversed(matches):
            val = self.parse_price("R$" + m)
            if val and val > 0:
                return val
    ...
    def _parse_product_jsonld(self, data):
        ...
        # Multiple offers = variant product, take first with positive price
        if isinstance(offers, list):
            price = None
            for offer in offers:
                try:
                    val = float(str(offer.get("price", "0")).replace(",", "."))
                    if val > 0:
                        price = val
                        break
    ...
    def _scrape_category(self, base_cat_url, tipo):
        all_products = []
        seen_urls = set()
        page = 1
        while True:
            ...
            logger.info(f"[ofertasna] {base_cat_url} page {page}: +{len(new)}")
            page += 1
            self.delay()
        return all_products

    def scrape(self):
        for cat_url, tipo in CATEGORIES:
            products = self._scrape_category(cat_url, tipo)
            self.result.products.extend(products)
            logger.info(f"[ofertasna] {cat_url}: {len(products)} total")
        return self.result
